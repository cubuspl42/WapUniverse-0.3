...
private class Y {
    ...
    class Root {
        var imageSets: Map<String, Y.ImageSet> = HashMap() // fully qualified imageset id -> imageset

        fun findImageSet(expandedImageSetId: String): Y.ImageSet? {
...
private fun readDatabase(yamlStream: InputStream): Y.Root {
    yamlStream.use {
        return yaml.load(it) as Y.Root
    }
}

class YamlRezMetadataDao(yamlStream: InputStream) : RezMetadataDao {
    private var root = readDatabase(yamlStream)
