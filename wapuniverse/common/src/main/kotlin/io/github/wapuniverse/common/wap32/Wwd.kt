...
import io.github.wapuniverse.common.util.flagProperty
...
class WwdInputStream(private val inputStream: InputStream) {
    fun expectInt(expectedValue: Int) {
        val value = readInt()
        if (value != expectedValue) throw IllegalStateException("${inputStream.toString()}: $value != $expectedValue")
    }
...
data class Wwd(
        ...
        var planes: MutableList<WwdPlane> = mutableListOf<WwdPlane>(),
        var tileDescriptions: MutableList<WwdTileDescription> = mutableListOf<WwdTileDescription>()
) {
...
data class WwdPlane(
        ...
        var tilesWide: Int = 0,
        var tilesHigh: Int = 0,
        var tiles: MutableList<Int> = mutableListOf<Int>(),
        var imageSets: MutableList<String> = mutableListOf<String>(),
        var objects: MutableList<WwdObject> = mutableListOf<WwdObject>()
) {
    companion object {
