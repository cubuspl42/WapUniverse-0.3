...
class ImageSetDatabase(yamlDocumentStream: InputStream) {
    ...
    private var root: Schema.Root = Schema.Root()

    init {
        val constructor = Constructor(Schema.Root::class.java)
        with(constructor) {
            addTypeDescription(TypeDescription(Schema.Image::class.java))
            addTypeDescription(TypeDescription(Schema.ImageSet::class.java))
        }
        yamlDocumentStream.use {
            val yaml = Yaml(constructor)
            root = yaml.load(it) as Schema.Root
        }
    }

    private fun findImageSet(expandedImageSetId: String): Schema.ImageSet? {
        val imageSet = root.imageSets[expandedImageSetId]
        return imageSet
    }

    private fun findImageMetadata(imageSet: Schema.ImageSet, frameIndex: Int): ImageMetadata? {
    ...
    private fun findTileImageSet(levelIndex: Int, tileImageSetName: String): Schema.ImageSet? {
